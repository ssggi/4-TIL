...
<br>
- 중요 매개변수는 n_estimators(트리의 개수를 지정), learning_rate(트리의 오차를 보정하는 정도를 조절) <br>
- n_estimators가 클수록 모델이 복잡해지고 과대적합될 가능성이 높아짐  <br>
- learning_rate를 낮추면 비슷한 복잡도의 모델을 많이 만들기 위해 더 많은 트리를 추가해야함 - 뭔 말이지?<br>
